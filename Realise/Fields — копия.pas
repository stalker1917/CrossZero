...
Figures{K,FiguresN} : Array of TFigure;
...
begin
   ...
   //Move(Figures[0],F.Figures[0],Length(Figures)*SizeOf(TFigure));
   //SetLength(F.FiguresN,Length(FiguresN));
  // Move(FiguresN[0],F.FiguresN[0],Length(FiguresN)*SizeOf(TFigure));
  {for I := 0 to GridSize-1 do
   for J := 0 to GridSize-1 do
     F.Data[i,j] := Data[i,j]; //Заменить на глубокое копирование. }
end;
  ...
  with Figures[i2] do
    repeat
      ...
      if Data[StartX+l*XIndex[Turn],StartY+l*YIndex[Turn]].Color=Color  then
        begin
          inc(m);
          RefreshFigure(StartX,StartY,Turn,l,i,i2);
          //Data[StartX+l*XIndex[Turn],StartY+l*YIndex[Turn]].Figure[Turn]:=i;
        end;
      inc(l);
    until m=Length;
  if l-m>1 then
        ...
        begin
           ...
           j2 := Data[X+XIndex[i],Y+YIndex[i]].Figure[k];
             if (j2>=0) and (i<4) then  //xvx  //and (CenterHole(X,Y,i))) then // xvx
               ...
               if j>=0 then Join(j,i,X,Y,1)
               else //SetFigures(X,Y,i,1);
...
begin
  ...
  { begin
      Data[Figures[i].StartX,Figures[i].StartY].Figure[j]:=-1;
      Data[Figures[i].StartX,Figures[i].StartY].Figure[k]:=-1;
    end; }
  ...
  begin
   ...
   for l := 0 to Buf.Length-1 do RefreshFigure(Buf.StartX,Buf.StartY,Turn,l,k,i);
    { begin
       Data[Buf.StartX+l*XIndex[j],Buf.StartY+l*YIndex[j]].Figure[j]:=i;
       Data[Buf.StartX+l*XIndex[j],Buf.StartY+l*YIndex[j]].Figure[k]:=i;
     end; }
  end
  else RefreshFigure(Buf.StartX,Buf.StartY,Turn,0,-1,i)
  {
    begin
      Data[Buf.StartX+XIndex[j],Buf.StartY+XIndex[j]].Figure[j]:=-1;
      Data[Buf.StartX+XIndex[j],Buf.StartY+YIndex[j]].Figure[k]:=-1;
    end;}
end;
...
begin
 ...
 for i := 0 to 7 do
    begin
      if OutOfBounds(X+XIndex[i],Y+YIndex[i]) then continue;
      if  abs(Data[X+XIndex[i],Y+YIndex[i]].Color-Data[X,Y].Color)>=2 then  //Действительно есть блорк
        begin
          k:=(i+4) mod 8;
...
begin
    begin
       ...
       for I := 0 to GridSize-1 do
         for J := 0 to GridSize-1 do
           if Data[i,j].Color=0 then
             begin
               for k := 0 to 3 do
                 begin
                   //ScoreP := 0;
                   //ScoreN := 0;
                   GetFreePlace(i,j,k,2,ScoreP);
                   GetFreePlace(i,j,k+4,2,ScoreN);
       ...
       if (Max.ScoreP+Max.ScoreN)>=4 then
         begin
           SetData(Max.X+XIndex[Max.Turn],Max.Y+YIndex[Max.Turn],0);
           exit;
         end;

       for I := 0 to GridSize-1 do
         for J := 0 to GridSize-1 do
           if Data[i,j].Color=2 then
             begin
               Zone := Random(2);
               ...
               if Data[i+XIndex[Zone],j+YIndex[Zone]].Color=1 then //Ставим только в пустую клетку
                 begin
                   SetData(i+XIndex[Zone],j+YIndex[Zone],0);
                   exit;
                 end;
             end;
...
YesTurn:Boolean;
begin
  if Length(MainField.Figures)=0 then MainField.FindFreeTurn
  else
    repeat
    YesTurn := True;
      for I := 0 to 2 do
      ...
      if (Max[0]<0) and (Max[2]<0) then MainField.FindFreeTurn
      else
...
begin
  result := 0;
  if Length(Field.Figures)=0 then Field.FindFreeTurn
  else
    begin//repeat
    YesTurn := True;
      for I := 0 to 2 do
      ...
      if (Max[0,0]<0) and (Max[2,0]<0) then 
        ...
        begin
         ...
         if (RecDeep<MaxDeep) and (Score[2]<10) and (Score[0]<10) then
           begin
             if Score[2]>Score[0] then YesTurn :=Field.AppendHard(Max[2],TurnColor,RecDeep+1,result)
             else YesTurn := Field.AppendHard(Max[0],TurnColor,RecDeep+1,result); 
           end;
         if (RecDeep=MaxDeep) or (Score[2]>=10) or (Score[0]>=10)  or (not YesTurn) then
          begin
            ...
            if (RecDeep=0) and (not YesTurn) then
             Field.FindFreeTurn;
          end;
        end;
    end;//until YesTurn;
